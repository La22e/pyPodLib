...
Cross-referenced against:
  - src/pypodlib/itunesdb_parser/mhbd_parser.py parse_db()
...
from pypodlib.device import ChecksumType, DeviceCapabilities, detect_checksum_type
...
from pypodlib.device.filesystem import (
    detect_filesystem_type,
    resolve_itunesdb_platform,
)
from pypodlib.device.path_safety import resolve_device_path
...
from pypodlib.device.write_guard import DeviceWriteSafetyError
from pypodlib.device.write_readiness import inspect_device_write_readiness
from pypodlib.itunesdb_shared.album_identity import album_identity_from_track
from pypodlib.itunesdb_shared.device_time import active_device_time_context
from pypodlib.itunesdb_shared.field_base import (
    read_fields,
    write_fields,
    write_generic_header,
)
from pypodlib.itunesdb_shared.mhbd_defs import (
    MHBD_HEADER_SIZE,
...
# ``pypodlib.device.DeviceCapabilities.db_version``.
...
def write_itunesdb(
    ...
    if pc_file_paths is not None:
        ...
        if capabilities is not None and not capabilities.supports_artwork:
            try:
                from pypodlib.device import ITHMB_FORMAT_MAP
        ...
        try:
            from pypodlib.artworkdb_writer.artwork_writer import PendingArtworkWrite, write_artworkdb
