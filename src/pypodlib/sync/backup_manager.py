...
from pypodlib.device.filesystem_profile import FilesystemProfile, inspect_filesystem_profile
from pypodlib.device.storage_safety import (
    allocated_size,
    existing_file_allocated_size,
    require_file_size_supported,
)
from pypodlib.device.virtual_identity import virtual_ipod_profile
...
def _resolve_default_backup_dir() -> str:
    try:
        from pypodlib.infrastructure.settings_paths import default_data_dir
...
def _repository_bootstrap_lock_identity(backup_root: Path) -> str:
    ...
    try:
        ...
        return f"pypodlib-backup-repository:bootstrap:path:{normalized}"
    ...
    return (
        "pypodlib-backup-repository:bootstrap:"
...
def _repository_lock_identity(backup_root: Path) -> str:
    ...
    if windows_identity is not None:
        ...
        return (
            "pypodlib-backup-repository:object:windows:"
    ...
    if not device and not inode:
        return _repository_bootstrap_lock_identity(resolved)
    return (
        "pypodlib-backup-repository:object:"
