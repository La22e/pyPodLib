...
Scrobbling rules (per LB docs):
  ...
  - Only submit iPod plays still pending in the durable scrobble queue.
...
from pypodlib.infrastructure.version import get_version
...
def build_scrobble_entries(
    ...
    iPod plays still pending in the durable ``play_count_2`` queue.
    ...
    *,
    ...
    Args:
        playcount_items: SyncItems with action=SYNC_PLAYCOUNT.
            Each item carries ``play_count_delta`` from the durable pending
            scrobble queue.
