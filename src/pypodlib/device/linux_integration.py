...
RULE_VERSION = "2"
RULE_FILENAME = "61-pypodlib.rules"
...
def udev_rule_text() -> str:
    ...
    resource = files("pypodlib").joinpath(*_RULE_PARTS)
...
RULE_TMP="$(mktemp "${{TMPDIR:-/tmp}}/pypodlib-udev.XXXXXX")" || \\
...
run_as_root udevadm test --action=change "$SYSPATH" 2>&1 \\
  | grep -Ei '61-pypodlib|pypodlib|scsi_id|apple|ipod|error|failed|invalid|unknown' \\
  >&2 || true
