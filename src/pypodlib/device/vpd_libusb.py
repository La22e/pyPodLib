...
Standalone (writes SysInfo to iPod)::
    ...
    # macOS / Linux
    sudo uv run python -m pypodlib.device.vpd_libusb [--write-sysinfo]

    # Windows (no elevation needed for query, may need admin for write)
    uv run python -m pypodlib.device.vpd_libusb [--write-sysinfo]

    # Any platform — manually specify iPod mount path
    sudo uv run python -m pypodlib.device.vpd_libusb --write-sysinfo --path /Volumes/IPOD

From code::

    from pypodlib.device.vpd_libusb import query_ipod_vpd, query_all_ipods
        ...
        try:
            ...
            if "Access denied" in str(exc) or "Operation not permitted" in str(exc):
                raise PermissionError(
                    ...
                    "Run with: sudo uv run python -m pypodlib.device.vpd_libusb"
                ) from exc
            raise
    ...
    *,
    ...
    if sys.platform == "darwin":
        try:
            ...
            logger.debug("_vpd_query_any_platform: pypodlib.device.vpd_iokit not available")
        except Exception as exc:
    ...
    if scsi_vpd is None and sys.platform == "win32" and mount_path:
        try:
            ...
            logger.debug(
                "_vpd_query_any_platform: pypodlib.device.vpd_windows not available",
            )
        except Exception as exc:
    ...
    if scsi_vpd is None and sys.platform == "linux" and mount_path:
        try:
            ...
            logger.debug("_vpd_query_any_platform: pypodlib.device.vpd_linux not available")
        except Exception as exc:
    ...
    if include_usb_vendor:
        try:
            ...
            logger.debug(
                "_vpd_query_any_platform: pypodlib.device.vpd_usb_control not available",
            )
        except Exception as exc:
